...
const video_msg_array = ['Creating quiz with the video...'];
...
export function QuickQuiz({ decodedUrl }: QuickQuizProps) {
  const [msg, setMsg] = useState('Creating quiz...');
  ...
  const { mutateAsync: uploadFileAndRedirect } = useCreateChat(
    async (data: { chat_id: string }) => {
      ...
      if (workspaceFileId) {
        ...
        router.push(`/?tab=Quiz&openQuiz=true&workspaceFileId=${workspaceFileId}`);
      } else {
        toast.error('An error occurred while creating flashcards. Please try again.');
      }
    },
  ...
  useEffect(() => {
    const uploadFileAndStartCreatingFlashcards = async () => {
      ...
      if (isValidFileUrl) {
        setMsg(`Creating Quiz with the ${acceptedFileTypesToFileName[fileType]}...`);
